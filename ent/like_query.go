...
func (_q *LikeQuery) QueryProject() *ProjectQuery {
...
func (_q *LikeQuery) WithProject(opts ...func(*ProjectQuery)) *LikeQuery {
	query := (&ProjectClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LikeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Like, error) {
	...
	if query := _q.withProject; query != nil {
		if err := _q.loadProject(ctx, query, nodes, nil,
			func(n *Like, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LikeQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Like, init func(*Like), assign func(*Like, *Project)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Like)
	for i := range nodes {
...
func (_q *LikeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProject != nil {
			_spec.Node.AddColumnOnce(like.FieldProjectID)
		}
	}
