...
type TagQuery struct {
	...
	withUsers            *UserQuery
	withProjectTags      *ProjectTagQuery
...
func (_q *TagQuery) QueryCreator() *UserQuery {
...
func (_q *TagQuery) QueryProjects() *ProjectQuery {
...
// QueryProjectTags chains the current query on the "project_tags" edge.
func (_q *TagQuery) QueryProjectTags() *ProjectTagQuery {
	query := (&ProjectTagClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TagQuery) QueryUserTechnologies() *UserTechnologyQuery {
...
func (_q *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withProjectTags:      _q.withProjectTags.Clone(),
...
func (_q *TagQuery) WithCreator(opts ...func(*UserQuery)) *TagQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *TagQuery) WithProjects(opts ...func(*ProjectQuery)) *TagQuery {
	query := (&ProjectClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithProjectTags tells the query-builder to eager-load the nodes that are connected to
// the "project_tags" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TagQuery) WithProjectTags(opts ...func(*ProjectTagQuery)) *TagQuery {
	query := (&ProjectTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withProjectTags = query
	return _q
}
...
func (_q *TagQuery) WithUserTechnologies(opts ...func(*UserTechnologyQuery)) *TagQuery {
	query := (&UserTechnologyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withCreator; query != nil {
		if err := _q.loadCreator(ctx, query, nodes, nil,
			func(n *Tag, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProjects; query != nil {
		if err := _q.loadProjects(ctx, query, nodes,
			func(n *Tag) { n.Edges.Projects = []*Project{} },
			func(n *Tag, e *Project) { n.Edges.Projects = append(n.Edges.Projects, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withProjectTags; query != nil {
		if err := _q.loadProjectTags(ctx, query, nodes,
			func(n *Tag) { n.Edges.ProjectTags = []*ProjectTag{} },
			func(n *Tag, e *ProjectTag) { n.Edges.ProjectTags = append(n.Edges.ProjectTags, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withUserTechnologies; query != nil {
		if err := _q.loadUserTechnologies(ctx, query, nodes,
			func(n *Tag) { n.Edges.UserTechnologies = []*UserTechnology{} },
			func(n *Tag, e *UserTechnology) { n.Edges.UserTechnologies = append(n.Edges.UserTechnologies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Tag)
	for i := range nodes {
		if nodes[i].user_created_tags == nil {
			continue
		}
		fk := *nodes[i].user_created_tags
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_tags" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TagQuery) loadProjects(ctx context.Context, query *ProjectQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Project)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ProjectsTable)
		s.Join(joinT).On(s.C(project.FieldID), joinT.C(tag.ProjectsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.ProjectsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ProjectsPrimaryKey[1]))
...
func (_q *TagQuery) loadProjectTags(ctx context.Context, query *ProjectTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *ProjectTag)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(projecttag.FieldTagID)
	}
	query.Where(predicate.ProjectTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.ProjectTagsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TagQuery) loadUserTechnologies(ctx context.Context, query *UserTechnologyQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *UserTechnology)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Tag)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usertechnology.FieldTechnologyID)
	}
	query.Where(predicate.UserTechnology(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.UserTechnologiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "technology_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
